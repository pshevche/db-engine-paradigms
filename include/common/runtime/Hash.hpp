...
#define IS_INT_LE(bits, t)                                                     \
   (sizeof(t) <= (bits / 8) && std::is_integral<t>::value)

#define IS_INT_EQ(bits, t)                                                     \
   (sizeof(t) == (bits / 8) && std::is_integral<t>::value)
// 32-bit hashes
class MurMurHash3 : public Hash<MurMurHash3> {
 public:
   ...
   template <class T>
   inline auto hashKey(T k, hash_t seed) const ->
       typename std::enable_if<IS_INT_LE(32, T), hash_t>::type {
      uint32_t h1 = seed;
   ...
   template <class T>
   inline auto hashKey(T k, hash_t seed) const ->
       typename std::enable_if<IS_INT_EQ(64, T), hash_t>::type {
      // inline hash_t hashKey(uint64_t k, hash_t seed) const {
      uint32_t h1 = seed;
      const uint32_t c1 = 0xcc9e2d51;
      const uint32_t c2 = 0x1b873593;
...
class CRC32Hash : public Hash<CRC32Hash> {
 ...
 public:
   ...
   // hashKey(T k, hash_t seed) const -> typename
   // std::enable_if<IS_INT_LE(32,T), hash_t>::type{
...
//Extending hashing function for simple multiplicative hashing.
// By doing this, we can simulate the behavior of vectorwise
EXTRAOPS(CRC32Hash);




class MultiplicativeHash : public Hash<MultiplicativeHash> {
