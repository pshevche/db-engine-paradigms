...
const char* CompilationEngine::precompileHeader() {
   ...
   std::string path_to_precompiled_header = path_to_minimal_api_header + ".pch";
...
const char* CompilationEngine::compileQ6CPP() {
   // generate code
...
const char* CompilationEngine::compileQ6LL() {
   // generate code
   const std::string& filename = "src/hybrid/typer_q6";
   std::ofstream f(filename + ".cpp");
   f << "#include \"hybrid/minimal_api.hpp\"\n";
   f << "using namespace runtime;\n";
   f << "using namespace std;\n";
   f << "Relation compiled_typer_q6(Database& db, size_t nrThreads, size_t "
