...
/*
 * This variant is a workaround for bad code generation of gcc. It is
 * semantically equivalent to hash4_sel<int32_t, DEFAULT_HASH>
 */
pos_t hash4_selASM(pos_t n, pos_t* RES inSel, hash_t* RES result,
                   int32_t* RES input)
/// compute hash for input column
{
   size_t rest = n % 8;
   Vec8u seeds(seed);
   asm("movabsq	$29875498475984, %%r8;"
       ...
       "cmpq	%%r11, %2;"
       "jne	.Hash4SelInner;" ::"r"(inSel), // pointer to selection vector
       "r"(input),                             // pointer to data for gathering
       "r"(result),                            // pointer to output vector
       "r"((n - rest) * 8)
       : "k1", "k2", "r11", "r10", "r8", "ymm1", "zmm0", "zmm1", "zmm2", "zmm3",
